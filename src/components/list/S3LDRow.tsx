    ...
    render() {
        const is_public = (this.props.type === S3ObjectType.object)
            ? <S3IsPublicIcon
                ...
                toggleVisibilityCB={this.props.toggleVisibilityCB}
            />
            : undefined;
        ...
        const entry = (this.props.type === S3ObjectType.folder)
            ? <a href={"#"} onClick={() => this.props.navigationCallback(this.props.object_key)}>{filename}</a>
            : <a href={"#"} onClick={() => this.props.openObjectCallback(this.props.object_key, this.props.name)}>{filename}</a>;
